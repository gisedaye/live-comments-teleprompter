...
function loadCommentsReactions() {
  ...
  const STREAMING_URL = 'https://streaming-graph.facebook.com/';

  //GD: without this the data wont come
  ...
  if (!window.shared.post){
    fbAPIPromise(
      ...
      post.created_time = moment(post.created_time);
  ...
  //GD: change the calls to get data from icebreaker
  var source = new EventSource(STREAMING_URL + encodeURIComponent(postId) + "/live_comments/?access_token=" + accessToken);
  source.onmessage = function(e) {
      let comment = JSON.parse(e.data);
      if (comment.hasOwnProperty('message') && !comment.message.trim()) { // Throw out messages with no text (usually photos, GIFs, etc.)
        return;
      }
      comments.merge({
        id: comment.from.id,
        created: moment(comment.created_time),
        name: (comment.from && comment.from.name) ? comment.from.name : 'Facebook User',
        message: comment.message
      }, 'id');
  };

  var source = new EventSource(STREAMING_URL + encodeURIComponent(postId) + "/live_reactions/?access_token=" + accessToken);
  source.onmessage = function(e) {
    let res = JSON.parse(e.data);
    if (res.hasOwnProperty('reaction_stream')) {
      res.reaction_stream.forEach((data) => {
        reactions.set(data.key, data.value);
      });
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  FB.Event.subscribe('auth.statusChange', (res) => {
    if (res.status === 'connected') {
      ...
      //GD: Change to get the access token to make the icebreaker calls
  ...
  document.querySelector('form[name="openUrl"]').addEventListener('submit', (e) => {
    ...
    new Promise((resolve, reject) => {
      ...
      return fbAPIPromise(
        ...
        // GD: changed for live just get the video id
        return match[2];
      });
  ...
  window.dispatchEvent(new Event('hashchange')); // Force a refresh of state
  loadCommentsReactions(); // Load fresh comments/reactions every few seconds
});
